...
export const Stopwatch = () => {
  const { deliveryStartTime, bodyOutTimes, apgar5MinParams, visualFlash, playChime, speakTime, startDelivery, stopDelivery } = useAppContext();
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const currentBlock = Math.floor(diff / 30000); 
  ...
  }, [deliveryStartTime, apgar5MinParams, bodyOutTimes, playChime, speakTime, visualFlash]);
  ...
  return (
    <>
      <div 
        className={`fixed inset-0 z-[999] pointer-events-none transition-opacity duration-1000 ease-out bg-indigo-500/30 dark:bg-indigo-500/40 ${isFlashing ? 'opacity-100' : 'opacity-0'}`}
      ></div>

      <div className="flex flex-col items-center justify-center py-2 w-full relative mt-2 sm:mt-4 z-10">
        ...
        {showResetConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm">
            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 max-w-sm w-full shadow-2xl relative overflow-hidden pointer-events-auto">
